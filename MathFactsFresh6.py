...
if 'level' not_in st.session_state:
    st.session_state.level = 1
if 'questions' not_in st.session_state:
    st.session_state.questions = []
if 'current_question' not_in st.session_state:
...
with st.form(key='answer_form', clear_on_submit=True):
    # Use a fixed key for simplicity and predictability
...
if submit:
    if answer.strip().isdigit():
        ...
        # Inject JavaScript to focus on the input field
        st.components.v1.html(
            """
            <script>
                // Wait for the DOM to update and focus the input
                setTimeout(() => {
                    const input = document.querySelector('input[name="math_answer_input"]');
                    if (input) {
                        input.focus();
                        console.log("Focused on input"); // Debugging in browser console
                    } else {
                        console.log("Input not found"); // Debugging
                    }
