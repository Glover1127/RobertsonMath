...
if 'question_index' not in st.session_state:
    st.session_state.question_index = 0  # Track question index for unique key
...
if st.session_state.current_question is None:
    ...
    st.session_state.question_index += 1  # Increment index for each new question
...
with st.form(key='answer_form', clear_on_submit=True):
    # Use a stable key based on level and question index
    answer = st.text_input("Enter your answer:", key=f"answer_{st.session_state.level}_{st.session_state.question_index}")
...
if submit and answer.strip().isdigit():
    ...
    # Inject JavaScript to focus on the input field after rerun
    st.components.v1.html(
        f"""
        <script>
            setTimeout(() => {{
                const input = document.getElementById('answer_{st.session_state.level}_{st.session_state.question_index + 1}');
